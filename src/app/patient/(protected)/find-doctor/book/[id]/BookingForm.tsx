...
import { lockSlot, unlockSlot } from "@/redux/features/appointment/appointmentThunk";
...
export default function BookingForm({ doctor }: { doctor: any }) {
    ...
    const handleProceedToPay = async () => {
        ...
        if (lockSuccess) {
            // Temporarily skip appointment creation and redirection
                    ...
                    {isLoadingSlots ? (
                        ...
                        <div className={`grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 gap-3 ${isSlotLocked ? "pointer-events-none opacity-75" : ""}`}>
                            ...
                            {allSlots.map((slotObj) => {
                                ...
                                const isLocked = status === "locked" || status === "Locked" || apiIsLocked === true;
                                const isBooked = status === "booked" || status === "Booked";
                                const isPst = status === "past";
                                const isDisabled = isLocked || isBooked || isPst;
                                const isSelected = time === slotTime && !isDisabled;
                                ...
                                return (
                                    <button
                                        ...
                                        title={
                                            isLocked ? "Locked by another user"
                                        ...
                                        `}
                                    >
                                        <span className={`text-sm font-bold flex items-center gap-1.5 ${isPst || isBooked || isLocked ? 'opacity-70' : ''}`}>
                                            {isLocked && <i className="fas fa-lock text-[10px] opacity-70" />}
                        ...
                        {modalError ? (
                            ...
                            <div className="p-6">
                                ...
                                <div className="flex gap-3">
                                    ...
                                    >
                                        {isLocking ? (
                                            <><div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" /> Locking...</>
                                        ) : (
                                            "Proceed to Pay"
                                        )}
                                    </button>
